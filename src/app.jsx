      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-1 bg-slate-100/80 p-1 rounded-2xl border border-slate-200/60">
          {[
            ...
            { id: 'analitica', label: 'Analítica & Varianza', icon: 'chart' },
...
const DashboardView = ({ setActiveTab, onOpenNewModal, onSelectAccountFilter, onEditModal }) => {
  ...
  return (
    <div className="space-y-6 pb-24 md:pb-8">
      ...
      {/* Grid de Cuentas Bancarias (Orden: BBVA, Santander, Sabadell Ahorro, Sabadell IRPF, Efectivo, Inversiones) */}
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {/* 1 al 5: Cuentas individuales ordenadas */}
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <span className="text-[11px] font-medium text-slate-400 capitalize">
                        {c.id === 'acc-bbva' ? 'Cuenta Principal' : c.tipo}
                      </span>
                    </div>
                  </div>
          ...
          {/* 6: Tarjeta Conjunta Unificada de Inversiones (MyInvestor + Trade Republic con Desplegable) */}
          <div className="group relative bg-white hover:bg-slate-50/80 p-5 rounded-2xl border border-purple-200/90 shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
            <div>
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2.5">
                  <span className="w-3.5 h-3.5 rounded-full bg-purple-600 ring-4 ring-purple-100"></span>
                  <div>
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex items-baseline justify-between">
                <span className="text-xs text-slate-400 font-medium">Saldo conjunto</span>
                <span className="text-xl font-extrabold font-sans text-purple-900">
                  {formatCurrency(totalInversion)}
                </span>
              </div>
            </div>

            {/* Desplegable interactivo para verlas por separado */}
            <div className="mt-3 pt-2.5 border-t border-slate-100">
              ...
              >
                <span>{isInvExpanded ? 'Ocultar desglose' : 'Ver cuentas por separado'}</span>
                <Icon name={isInvExpanded ? 'chevronUp' : 'chevronDown'} className="w-3.5 h-3.5 text-purple-600" />
              </button>

              {isInvExpanded && (
                <div className="mt-2 space-y-1.5 pt-1.5 border-t border-purple-100/60 animate-fadeIn">
                  <div
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold text-slate-800">MyInvestor</span>
                    </div>
                    <span className="text-xs font-bold text-purple-900 font-sans">
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold text-slate-800">Trade Republic</span>
                    </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Cobro en Santander, aportación a MyInvestor y disponible restante a gastos.
          </p>
        </div>
      ...
      {distributionResult && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 relative animate-fadeIn">
          ...
          <p className="text-xs text-emerald-700 mb-3">
            Se han añadido los ingresos en Santander y generado las transferencias automáticas para la fecha {formatDate(distributionResult.fecha)}.
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        <div className="md:col-span-7 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-4">
          ...
          <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
            <span className="text-xs text-slate-500 font-medium">Cuenta donde se cobra el sueldo:</span>
            <select
              value={cuentaIngreso}
            ...
            >
              {sortedCuentas.filter(c => c && c.activa).map(c => (
        ...
        <div className="md:col-span-5 bg-gradient-to-br from-slate-900 to-slate-850 text-white p-6 rounded-3xl shadow-md flex flex-col justify-between space-y-4">
          <div>
            ...
            <p className="text-xs text-slate-400">Total nóminas a distribuir desde {sortedCuentas.find(c => c.id === cuentaIngreso)?.nombre || 'Santander'}</p>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Elige la cuenta bancaria de cargo (BBVA predeterminada) y pulsa "Pagar" el día que te pasen el recibo.
            </p>
          </div>
              ...
              >
                <div>
                  ...
                  <div className="mt-2 flex items-center justify-between gap-1 text-[11px]">
                    ...
                    >
                      {sortedCuentas.filter(c => c.activa).map(c => (
                        <option key={c.id} value={c.id}>{c.nombre}</option>
                      ))}
                    </select>
                  </div>
...
const AnaliticaView = () => {
  ...
  const donutSlices = useMemo(() => {
    const { list, totalGasto } = categoryDistribution;
  ...
  const monthlyVarianceHistory = useMemo(() => {
    ...
    allMonths.forEach(mKey => {
      ...
      if (selectedAccountScope === 'total-liquido') {
        currentScopeSaldo = (data.cuentas || [])
          .filter(c => c && c.activa && c.incluirEnTotal !== false)
          .reduce((sum, c) => sum + (runningBal[c.id] || 0), 0);
      } else if (selectedAccountScope === 'total-consolidado') {
        currentScopeSaldo = (data.cuentas || [])
          .filter(c => c && c.activa)
          .reduce((sum, c) => sum + (runningBal[c.id] || 0), 0);
  ...
  const chartGraphData = useMemo(() => {
    if (monthlyVarianceHistory.length === 0) return { path: '', area: '', points: [], width: 700, height: 260 };
    ...
    return { path, area, points, width, height, minVal, maxVal, effectiveMin, effectiveMax };
    ...
    <div className="space-y-6 pb-24 md:pb-8">
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-50 text-blue-800 border border-blue-200 mb-1">
            <Icon name="chart" className="w-3.5 h-3.5 text-blue-600" />
            Recorrido Histórico & Varianza Mes a Mes
          </div>
          <h2 className="text-xl font-bold text-slate-900">Analítica & Varianza por Cuenta</h2>
          <p className="text-xs text-slate-500">
            Observa el saldo con el que cerraste cada mes, las variaciones netas y el recorrido de tus cuentas.
          </p>
        </div>
      ...
      <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/80 shadow-sm flex flex-wrap items-center justify-between gap-4">
        <div className="flex flex-wrap items-center gap-3">
          <div>
            <label className="text-[11px] font-bold text-slate-500 block mb-1">Cuenta / Ámbito a analizar:</label>
            <select
            ...
            >
              <optgroup label="Totales">
                <option value="total-liquido">💰 Patrimonio Líquido (Operativo)</option>
                <option value="total-consolidado">🌐 Total Consolidado (Con IRPF e Inversión)</option>
                <option value="inversiones-total">💼 Cartera Inversión (MyInvestor + Trade)</option>
              </optgroup>
              <optgroup label="Cuentas Individuales">
                {sortedCuentas.map(c => (
                  <option key={c.id} value={c.id}>
                    {c.nombre} {c.incluirEnTotal === false ? '(Separada)' : ''}
                  </option>
                ))}
              </optgroup>
            </select>
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-blue-50 text-blue-800 border border-blue-200">
            {monthlyVarianceHistory.length} Meses computados
          </span>
        </div>
        ...
        >
          ...
          >
            ...
            {/* Líneas de cuadrícula con espacio superior */}
            <line x1="50" y1="50" x2="650" y2="50" stroke="#f1f5f9" strokeWidth="1" strokeDasharray="3 3" />
            <line x1="50" y1="130" x2="650" y2="130" stroke="#f1f5f9" strokeWidth="1" strokeDasharray="3 3" />
            <line x1="50" y1="215" x2="650" y2="215" stroke="#e2e8f0" strokeWidth="1" />

            {/* Área y Línea */}
            {chartGraphData.area && <path d={chartGraphData.area} fill="url(#varianceGrad)" />}
            {chartGraphData.path && <path d={chartGraphData.path} fill="none" stroke="#2563eb" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" />}
            ...
            {/* Puntos / Bolitas con interactividad */}
            {chartGraphData.points.map((p, i) => {
              ...
              return (
                ...
                >
                  {/* Halo invisible para facilitar hover táctil */}
                  <circle cx={p.x} cy={p.y} r="20" fill="transparent" />

                  {/* Halo visible al seleccionar */}
                  {isHovered && (
                    <circle cx={p.x} cy={p.y} r="10" fill="#93c5fd" opacity="0.6" className="animate-ping" />
                  )}
      ...
      {/* 🍩 DIAGRAMA CIRCULAR DE GASTOS POR CATEGORÍA CON TOOLTIPS FLOTANTES */}
      <div
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
              <Icon name="pieChart" className="w-4 h-4 text-purple-600" />
              Distribución de Gastos por Categoría
            </h3>
            <p className="text-xs text-slate-500">
              Porciones de gasto según el período seleccionado ({selectedMonth !== 'todos' ? formatMonthName(selectedMonth) : selectedYear !== 'todos' ? `Año ${selectedYear}` : 'Histórico Total'})
            </p>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-purple-50 text-purple-800 border border-purple-200">
              Total Gastos: {formatCurrency(categoryDistribution.totalGasto)}
            </span>
          </div>
        </div>

        {categoryDistribution.totalGasto <= 0 ? (
          <div className="p-10 text-center text-slate-400 bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-xs">
            No hay registros de gastos en este período para la cuenta seleccionada.
          </div>
        ) : (
                ...
                >
                  <div className="font-bold text-slate-200 border-b border-slate-800 pb-1 mb-1.5 flex items-center justify-between gap-3">
                    <span className="flex items-center gap-1.5">
                      <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: hoveredCategorySlice.color }}></span>
                      {hoveredCategorySlice.categoria}
                    </span>
                    <span className="text-[10px] px-1.5 py-0.5 rounded-md bg-slate-800 text-slate-300 font-mono font-bold">
                      {hoveredCategorySlice.porcentaje.toFixed(1)}%
                    </span>
                  </div>
                    ...
                    >
                      <div className="flex items-center justify-between text-xs mb-1.5">
                        <div className="flex items-center gap-2 font-bold text-slate-900">
                          ...
                          <span className="text-[10px] text-slate-400 font-normal">({item.count} movs)</span>
                        </div>
                ...
                displayedVarianceRows.map(row => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-3.5 text-right font-bold text-slate-900 font-sans">
                        {formatCurrency(row.saldo)}
                      </td>
                      <td className="p-3.5 text-right font-sans font-bold">
                        {row.prevSaldo === null ? (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-300">
                    Ámbito: {scopeLabel} • Año: {selectedYear}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 space-y-6 overflow-y-auto">
              <div className="p-4 bg-blue-50/70 border border-blue-200/80 rounded-2xl space-y-2">
                ...
                <p className="text-xs text-blue-950 leading-relaxed">
                  Tu patrimonio líquido disponible operativo asciende a <strong>{formatCurrency(totalPatrimonio)}</strong>, complementado con <strong>{formatCurrency(totalInversion)}</strong> en tu cartera de inversión conjunta (MyInvestor + Trade Republic) y <strong>{formatCurrency(saldos['acc-sab-irpf'] || 0)}</strong> en Sabadell IRPF, sumando un patrimonio total consolidado de <strong>{formatCurrency(totalPatrimonioAbsoluto)}</strong>.
                </p>
              </div>

              <div className="space-y-2">
                <h4 className="text-xs font-bold text-slate-900">Desglose por Cuentas Actualizadas</h4>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  {sortedCuentas.map(c => (
                    <div key={c.id} className="p-3 bg-slate-50 rounded-xl flex items-center justify-between">
                      ...
                      <span className="font-extrabold text-slate-900 font-sans">{formatCurrency(saldos[c.id] || 0)}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1 border-t border-slate-100">
            <div>
              ...
              <select
                ...
                className="w-full text-xs font-bold bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-slate-900"
              >
                {sortedCuentas.filter(c => c && c.activa && c.tipo === 'inversion').map(c => (
                  <option key={c.id} value={c.id}>{c.nombre}</option>
                ))}
              </select>
            </div>
            <div>
              <label className="text-xs font-semibold text-slate-700 block mb-1">Inversión Mensual (MyInvestor €)</label>
              <input
                type="number"
                step="10"
                value={invFija}
